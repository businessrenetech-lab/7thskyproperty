...
import{_ as k}from"./index-ZQ1Yah8X.js";import{l as T}from"./logo-WQCnS321.js";const $=async()=>(await k(async()=>{const{default:t}=await import("./html2pdf-CU0lLxV3.js").then(e=>e.h);return{default:t}},__vite__mapDeps([0,1]))).default,v=()=>({name:"Language Academy",address:"SEL SUFI SQUARE, Unit: 1104, Level: 11, Dhanmondi R/A, Dhaka 1209",phone:"+880 1913-373581",email:"hello@languageacademy.com.bd",website:"languageacademy.com.bd"});let w=null;const z=()=>new Promise(t=>{if(w){t(w);return}const e=new Image;e.crossOrigin="anonymous",e.onload=()=>{const o=document.createElement("canvas");o.width=e.width,o.height=e.height,o.getContext("2d").drawImage(e,0,0),w=o.toDataURL("image/png"),t(w)},e.onerror=()=>t(""),e.src=T}),_=t=>{if(t===0)return"Zero Taka Only";const e=["","One ","Two ","Three ","Four ","Five ","Six ","Seven ","Eight ","Nine ","Ten ","Eleven ","Twelve ","Thirteen ","Fourteen ","Fifteen ","Sixteen ","Seventeen ","Eighteen ","Nineteen "],o=["","","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"],i=("000000000"+Math.floor(Math.abs(t))).substr(-9).match(/^(\d{2})(\d{2})(\d{2})(\d{1})(\d{2})$/);if(!i)return String(t)+" Taka Only";let r="";return r+=i[1]!=0?(e[Number(i[1])]||o[i[1][0]]+" "+e[i[1][1]])+"Crore ":"",r+=i[2]!=0?(e[Number(i[2])]||o[i[2][0]]+" "+e[i[2][1]])+"Lakh ":"",r+=i[3]!=0?(e[Number(i[3])]||o[i[3][0]]+" "+e[i[3][1]])+"Thousand ":"",r+=i[4]!=0?(e[Number(i[4])]||o[i[4][0]]+" "+e[i[4][1]])+"Hundred ":"",r+=i[5]!=0?(r!==""?"and ":"")+(e[Number(i[5])]||o[i[5][0]]+" "+e[i[5][1]]):"",r.trim()+" Taka Only"},B=async(t,e)=>{const o=v(),i=await z();return`
  ...
  `},C=(t=["Created by","Received by","Approved by"])=>`
  ...
  `},j=async t=>{const e=await E(t),o=document.createElement("div");o.innerHTML=e,o.style.background="white";const i={margin:[6,6,6,6],filename:`Receipt-${t.receipt_no||t.id}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff"},jsPDF:{unit:"mm",format:"a5",orientation:"landscape"}};await(await $())().set(i).from(o).save()},I=async t=>{const e=v(),o=await z(),i=parseFloat(t.amount||0),r=parseFloat(t.paid||0),m=i-r,s=_(Math.floor(i)),l=t.issued_at?new Date(t.issued_at).toLocaleDateString("en-GB",{dateStyle:"medium"}):new Date().toLocaleDateString("en-GB",{dateStyle:"medium"}),b=t.due_date?new Date(t.due_date).toLocaleDateString("en-GB",{dateStyle:"medium"}):"N/A",n=t.invoice_type==="custom",f=n?t.customer_name||t.Customer?.name||"Customer":t.Student?.User?.name||t.Enrollment?.Student?.User?.name||"Student",y=n&&(t.customer_company||t.Customer?.company)||"",a=n&&(t.customer_phone||t.Customer?.phone)||"",x=n?t.customer_email||t.Customer?.email||"":t.Student?.User?.email||"",c=n&&(t.customer_address||t.Customer?.address)||"",u=n?t.IncomeCategory?.name||"Custom Income":t.Enrollment?.Batch?.Course?.title||"Tuition Fee",p={paid:"#10b981",pending:"#f59e0b",overdue:"#ef4444",partial:"#3b82f6",draft:"#64748b"}[t.status]||"#64748b";return`
    <div style="width:100%; font-family:'Inter','Segoe UI',sans-serif; position:relative; overflow:hidden; background:#ffffff; padding:0;">
      ...
      <div style="padding:24px 32px; position:relative;">
        ...
        <!-- Signatures -->
        ${C(["Prepared by","Received by","Authorized by"])}
      </div>
  ...
  `},O=async t=>{const e=await I(t),o=document.createElement("div");o.innerHTML=e,o.style.background="white";const i={margin:[8,8,8,8],filename:`Invoice-${t.invoice_no||t.id}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff"},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"}};await(await $())().set(i).from(o).save()},L=async t=>{const e=v(),o=await z(),i=parseFloat(t.amount||0),r=_(Math.floor(i)),m=`VCH-${t.id}-${Date.now().toString().slice(-4)}`,s=t.date?new Date(t.date).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"}):new Date().toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"}),l=t.Branch?.name||t.branch_name||"Head Office",b=t.category||"Office Expense",n=(t.payment_method||"cash").replace(/_/g," ").toUpperCase(),f=t.description||"Office Expense";return`
    <div style="width:100%; font-family:'Inter','Segoe UI',sans-serif; position:relative; overflow:hidden; background:#ffffff; padding:0;">
      ...
      <div style="padding:18px 28px; position:relative;">
        ...
        <!-- Signatures -->
        ${C(["Created by","Checked by","Approved by"])}
      </div>

      <!-- Bottom Accent Bar -->
      <div style="height:3px; background:linear-gradient(90deg, #7bc62e, #275fa7); margin-top:8px;"></div>
    </div>
  `},H=async t=>{const e=await L(t),o=document.createElement("div");o.innerHTML=e,o.style.background="white";const i={margin:[6,6,6,6],filename:`Voucher-${t.id}.pdf`,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:!0,backgroundColor:"#ffffff"},jsPDF:{unit:"mm",format:"a5",orientation:"landscape"}};await(await $())().set(i).from(o).save()},G=async(t,e)=>{const o=v(),i=e?.from&&e?.to?`Period: ${e.from} to ${e.to}`:`Generated: ${new Date().toLocaleDateString("en-GB",{dateStyle:"medium"})}`,r=await B("Expense Report",i),m=t.reduce((a,x)=>a+(x.status==="deleted"?0:parseFloat(x.amount||0)),0),s="padding:10px 12px; font-size:10px; font-weight:700; color:#fff; text-transform:uppercase; letter-spacing:0.5px; border-bottom:2px solid #1e4d8a; white-space:nowrap;",l=t.map((a,x)=>{const c=a.status==="deleted",u=c?"line-through":"none",p=c?"#94a3b8":"#334155";return`
