...
export default function SalesContacts({ scope }) {
  ...
  const isInteriorScope = scope === 'interior' || queryParams.get('scope') === 'interior' || location.pathname.includes('/residential-interior-design');
  const isOpsServiceScope = Boolean(scope && !['sales', 'buy', 'rental'].includes(scope)) || location.pathname.includes('/residential-interior-design') || location.pathname.includes('/water-tank') || location.pathname.includes('/air-conditioning') || location.pathname.includes('/short-stay');

  const svcBase = useCallback(() => {
    const p = location.pathname || '';
    const bases = ['/residential-interior-design', '/water-tank', '/air-conditioning', '/short-stay', '/property-care-concierge', '/land-property-assessment', '/loan-financial-support', '/property-documentation-verification', '/property-will-succession', '/removal-relocation'];
