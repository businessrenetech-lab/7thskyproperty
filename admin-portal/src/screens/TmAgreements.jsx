      ...
      {loading ? <div style={{ padding: 48, textAlign: 'center' }}><Spinner /></div> : (
        <div className="pm-card"><div className="pm-card-body" style={{ padding: 0 }}>
          <table className="pm-tbl">
            <thead><tr><th>Reference</th><th>Tenant</th><th>Contract value</th><th>Status</th><th style={{ textAlign: 'right' }}>Action</th></tr></thead>
            <tbody>
              {list.map((a) => (
                <tr key={a.id}>
...
function Builder({ onDone, onCancel }) {
  ...
  const send = async () => {
    ...
    if (!d.client.email) return toast.error('Enter the tenant email (Step 1)');
    setBusy(true);
    try {
      const body = { ...d, pricing_input: { ...d.pricing_input, monthly_rent: d.schedule_b.monthly_rent } };
      const r = await api.post('/rptm/agreements', body);
      setSent(r.data); toast.success('Agreement sent to tenant for signature');
  ...
  return (
    <div className="pm-scope">
      <div className="pm-head">
        <div><div className="pm-eyebrow">Agreements</div><h1>New Tenancy Management Agreement</h1><div className="pm-meta">Step {step + 1} of {STEPS.length} · {STEPS[step]}</div></div>
      ...
      <div className="pm-card"><div className="pm-card-body" style={{ padding: 20 }}>
        ...
        {step === 5 && (
          <div>
            ...
            <button className="pm-btn primary" disabled={busy} style={{ marginTop: 14 }} onClick={send}><Send size={15} /> {busy ? 'Sending…' : 'Send to tenant for signature'}</button>
          </div>
        )}
      </div></div>
