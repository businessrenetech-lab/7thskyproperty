...
function AgreementRegister() {
  ...
  return <>
    <WtHead title="Provider Master Agreements" subtitle="Canonical 63-clause agreement · approved rates · ordered two-party e-signature" search={q} onSearch={setQ}>
      ...
      <button className="wt-btn primary" onClick={() => nav('/water-tank/agreements/provider/new')}><Plus size={14} /> New agreement</button>
    </WtHead>
    ...
    <div className="wt-card wt-tblcard">{loading ? <Loading /> : shown.length ? <table className="wt-tbl"><thead><tr><th>Agreement</th><th>Provider</th><th>Version</th><th>Effective term</th><th>Rates</th><th>Signatures</th><th>Status</th><th /></tr></thead><tbody>{shown.map((row) => <tr className="click" key={row.id} onClick={() => nav(`/water-tank/agreements/provider/${row.code}`)}><td className="id">{row.code}</td><td><strong>{row.provider?.business_name || 'Provider unavailable'}</strong><div className="cell-sub">{row.provider?.code}</div></td><td>v{row.version_no}</td><td>{dateFmt(row.effective_date)}<div className="cell-sub">to {dateFmt(row.expiry_date)}</div></td><td>{Array.isArray(row.authorised_services) ? row.authorised_services.length : '—'} services</td><td>{(row.signers || []).filter((s) => s.status === 'signed').length}/{(row.signers || []).length || 2}</td><td><Pill value={row.status} sm /></td><td><ChevronRight size={15} /></td></tr>)}</tbody></table> : <EmptyState eyebrow="Provider agreements" title="No agreements found" hint="Start from a provider file or create a provider-prefilled agreement here." action={<button className="wt-btn primary" onClick={() => nav('/water-tank/agreements/provider/new')}><Plus size={14} /> New agreement</button>} />}</div>
  </>;
}
...
function AgreementBuilder({ id }) {
  ...
  const save = async () => {
    ...
    try { const { data } = agreement ? await api.patch(`/wt-agreements/provider/agreements/${agreement.code}`, payload()) : await api.post('/wt-agreements/provider/agreements', payload()); const row = data.agreement; setAgreement(row); toast.ok(`Draft ${row.code} saved`); if (!agreement) nav(`/water-tank/agreements/provider/${row.code}/edit`, { replace: true }); return row; }
    catch (e) { setError(errText(e, 'Could not save the agreement draft')); return null; } finally { setBusy(false); }
  };
  const send = async () => { const row = agreement || await save(); if (!row) return; setBusy(true); try { await api.post(`/wt-agreements/provider/agreements/${row.code}/send`); toast.ok('Agreement sent to the provider for the first signature'); nav(`/water-tank/agreements/provider/${row.code}`); } catch (e) { setError(errText(e, 'Could not send the agreement')); setBusy(false); } };
  ...
  return <>
    <WtHead crumb={<div className="wt-crumb"><span className="lnk" onClick={() => nav('/water-tank/agreements/provider')}>Provider Agreements</span> › <span>{agreement?.code || 'New draft'}</span></div>} title={agreement ? `Edit ${agreement.code}` : 'Draft Provider Master Agreement'} subtitle={`Canonical 63 clauses · Step ${step + 1} of ${STEPS.length} · ${STEPS[step]}`}>
      <button className="wt-btn" onClick={() => nav('/water-tank/agreements/provider')}>Cancel</button><button className="wt-btn" disabled={busy || !provider} onClick={save}><Save size={14} /> Save draft</button>
    </WtHead>
...
function AgreementDetail({ id }) {
  ...
  return <><WtHead crumb={<div className="wt-crumb"><span className="lnk" onClick={() => nav('/water-tank/agreements/provider')}>Provider Agreements</span> › <span>{agreement.code}</span></div>} title={agreement.code} subtitle={`${provider?.business_name || ''} · version ${agreement.version_no}`}>
    {agreement.status === 'Draft' && <button className="wt-btn primary" onClick={() => nav(`/water-tank/agreements/provider/${agreement.code}/edit`)}><FileSignature size={14} /> Edit draft</button>}
    ...
    {agreement.status === 'Completed' && <button className="wt-btn primary" onClick={() => nav(`/water-tank/agreements/provider/new?provider=${provider.code}&supersedes=${agreement.id}`)}><RefreshCw size={14} /> Amend or renew</button>}
  </WtHead>
