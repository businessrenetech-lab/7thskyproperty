...
function scheduleAFromCodes(codes = []) {
  const out = new Set();
  codes.forEach((code) => (CODE_TO_SCHEDULE_A[String(code).toUpperCase()] || []).forEach((n) => out.add(n)));
  return [...out];
}
...
async function computePricing(input = {}, branchId) {
  ...
  if (input.payment_overrides && input.payment_overrides.length) {
    ...
    const progress = Math.round(total * 0.3);
    payment_schedule = [
    ...
    summary.advance_percent = total > 0 ? 40 : 0;
...
function buildAgreement(data = {}) {
  ...
  const servicesSet = new Set([...(data.services || []), ...scheduleAFromCodes(pricedCodes)]);
  const checklistSet = new Set(data.checklist || []);
  const pricing = data.pricing || { lines: [], summary: {}, payment_schedule: [] };
  const doc_no = 'SS-WTCM-CSA-01';
  const title = 'Water Tank Cleaning & Maintenance — Customer Service Agreement';
  ...
  const schedA = scheduleChecks('sched-a', 'SCHEDULE A — Selected Services', SERVICE_GROUPS, servicesSet);
  ...
  const schedB = `<h2 id="sched-b" style="font-size:15px;color:#003768;margin:22px 0 6px;">SCHEDULE B — Project Summary</h2>${kvTable([
    ...
    ['Tank Type', b.tank_type], ['Tank Capacity', b.tank_capacity], ['Number of Tanks', b.tanks_count],
    ['Water Source', b.water_source],
  ...
  const schedD = scheduleChecks('sched-d', 'SCHEDULE D — Warranty Summary', CHECKLIST_GROUPS, checklistSet);
