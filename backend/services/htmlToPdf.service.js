 ...
 */
const fs = require('fs');
const { PRINT_STYLE } = require('./agreementPrintCss');
...
async function htmlToPdf(html) {
  ...
  try {
    ...
    // Same shared print stylesheet the served HTML uses, so a browser-printed
    // copy and this server-rendered PDF paginate identically. Margins match the
    // stylesheet's @page (16mm/14mm).
    await page.setContent(PRINT_STYLE + (html || ''), { waitUntil: 'networkidle0', timeout: 30000 });
