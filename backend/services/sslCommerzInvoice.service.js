...
async function initiateInvoiceCollection({ invoice, contact }) {
  const cfg = await config();
  const balance = Number(invoice.balance != null ? invoice.balance : (Number(invoice.total || 0) - Number(invoice.amount_paid || 0)));
  ...
  const transactionId = `SSPCINV${invoice.id}${Date.now().toString().slice(-8)}`.slice(0, 30);
  const base = `${cfg.callbackBase}/api/invoice-pay/sslcommerz`;
  const payload = new URLSearchParams({
    ...
    shipping_method: 'NO',
    product_name: (invoice.title || `Invoice ${invoice.invoice_code}`).slice(0, 120),
    product_category: invoice.invoice_type || 'invoice', product_profile: 'non-physical-goods',
    value_a: String(invoice.id), value_b: String(invoice.invoice_code || ''),
  });
