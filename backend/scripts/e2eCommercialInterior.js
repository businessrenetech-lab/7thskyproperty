...
const SL = 'commercial_interior_design';
...
(async () => {
  console.log(`\n===== COMMERCIAL INTERIOR E2E via COOKIE SESSION (run ${STAMP}) =====\n`);
  ADMIN = await loginCookie(EMAIL, PASSWORD);
  if (!ok(!!ADMIN, 'admin cookie session')) return finish();

  // ── Client on the interior line ─────────────────────────────────────────────
  const cl = await A('POST', '/api/wt-clients', { name: `Commercial Client ${STAMP}`, mobile: `0171${STAMP}`, email: `intcl${STAMP}@example.com`, service_address: 'Gulshan, Dhaka', property_type: 'Apartment' });
  const client = cl.body?.data || cl.body;
  const clientCode = client?.code;
  if (!ok([200, 201].includes(cl.status) && /^CIDS-C/.test(clientCode || ''), 'commercial client created (CIDS-C code)', `HTTP ${cl.status} code=${clientCode}`)) return finish();
  ...
  const csa = await A('POST', '/api/wt-agreements/customer/agreements', {
    ...
    pricing_input: { selected: [{ code: 'CIDS-103', qty: 1, agreed_price: 40000 }, { code: 'CIDS-201', qty: 1, agreed_price: 260000 }], vat_percent: 0 },
  });
  ...
  ok(/CIDS/.test(csa.body?.envelope_code || ''), 'envelope code carries the CIDS tag', csa.body?.envelope_code);
  ...
  const invoices = list((await A('GET', '/api/wt-invoices?limit=200')).body)
    .filter((i) => i.client_name === client.name || /^CIDI-/.test(i.code || ''));
  ...
  ok([200, 201].includes(v.status) && /^CIDW-V-/.test(vcode || ''), 'variation created (CIDW-V- code)', `HTTP ${v.status} ${vcode}`);
  ...
  // ── Cross-line isolation: water_tank must not see the commercial client ────────
  const wtClients = list((await AL('GET', '/api/wt-clients?limit=500', 'water_tank')).body);
  ok(!wtClients.some((c) => c.code === clientCode), 'commercial client NOT visible on the water_tank line', `wt clients=${wtClients.length}`);
