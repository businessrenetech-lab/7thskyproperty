...
const SL = 'furniture_styling_consultation';
...
(async () => {
  ...
  const cl = await A('POST', '/api/wt-clients', { name: `Furniture Styling Client ${STAMP}`, mobile: `0171${STAMP}`, email: `intcl${STAMP}@example.com`, service_address: 'Gulshan, Dhaka', property_type: 'Apartment' });
  const client = cl.body?.data || cl.body;
  const clientCode = client?.code;
  if (!ok([200, 201].includes(cl.status) && /^FSCS-C/.test(clientCode || ''), 'furniture client created (FSCS-C code)', `HTTP ${cl.status} code=${clientCode}`)) return finish();
  ...
  const csa = await A('POST', '/api/wt-agreements/customer/agreements', {
    ...
    pricing_input: { selected: [{ code: 'FSCS-103', qty: 1, agreed_price: 40000 }, { code: 'FSCS-201', qty: 1, agreed_price: 260000 }], vat_percent: 0 },
  });
  ...
  ok(/FSCS/.test(csa.body?.envelope_code || ''), 'envelope code carries the FSCS tag', csa.body?.envelope_code);
  ...
  const invoices = list((await A('GET', '/api/wt-invoices?limit=200')).body)
    .filter((i) => i.client_name === client.name || /^FSCI-/.test(i.code || ''));
  ...
  ok([200, 201].includes(v.status) && /^FSCW-V-/.test(vcode || ''), 'variation created (FSCW-V- code)', `HTTP ${v.status} ${vcode}`);
  ...
  // ── Cross-line isolation: water_tank must not see the furniture client ────────
  const wtClients = list((await AL('GET', '/api/wt-clients?limit=500', 'water_tank')).body);
  ok(!wtClients.some((c) => c.code === clientCode), 'furniture client NOT visible on the water_tank line', `wt clients=${wtClients.length}`);
