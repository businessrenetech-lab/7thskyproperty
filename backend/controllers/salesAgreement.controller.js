...
exports.createAgreement = asyncHandler(async (req, res) => {
  ...
  const out = await sequelize.transaction(async (t) => {
    ...
    // EnvelopeSigner.role is a fixed ENUM without buyer/seller; the party
    // identity is carried by the envelope kyc_role (buyer/seller) + related_type
    // + the signature-field label. Use the generic 'client' role here.
    const signer = await EnvelopeSigner.create({
      envelope_id: env.id, signer_order: 1, role: 'client',
