...
const Property = sequelize.define('Property', {
  ...
  unit_floor_plans: {
    ...
    get() { const v = this.getDataValue('unit_floor_plans'); if (typeof v === 'string') { try { return JSON.parse(v); } catch { return []; } } return v || []; },
  },
  ...
  // Structured wizard fields (0038). JSON columns can round-trip as strings — parse-safe getters.
  utilities: {
    type: DataTypes.JSON, defaultValue: [],
    get() { const v = this.getDataValue('utilities'); if (typeof v === 'string') { try { return JSON.parse(v); } catch { return []; } } return v || []; },
  },
  access_contacts: {
    type: DataTypes.JSON, defaultValue: [],
    get() { const v = this.getDataValue('access_contacts'); if (typeof v === 'string') { try { return JSON.parse(v); } catch { return []; } } return v || []; },
  },
  ownership_type: DataTypes.ENUM('sole', 'joint'),
  drawing_rooms: DataTypes.INTEGER,
  dining_rooms: DataTypes.INTEGER,
