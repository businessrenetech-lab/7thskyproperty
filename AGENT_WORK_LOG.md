...
### 2026-09-10 18:32 | OpenCode (GPT-6 Astra) | COMPLETED | Independent residential sales phased plan and Claude plan review
- Request: Continue research under Downloads/readit.txt, review Downloads/claude.md, and produce an independent practical phased improvement plan before implementation.
- Scope: Original six purchase/sale Office files, current residential routes/screens, sales and deal settlement paths, contacts, agreements/signing activation, accounting, communications, marketing and service integration patterns; official SaaS product research.
- Changes: Created `PROPERTY_SALES_SAAS_PHASED_PLAN.md`; appended this coordination entry. Temporary read-only workbook schema extraction script stored under approved opencode temp directory. No application source, DB, migration, distribution or original plan/document edits.
- Decisions: Retain the stronger sales settlement controls; connect the separate deal fee/commission flow through explicit financial links; distinguish coordination-only/external funds from client-money custody; separate brokerage profit from client property gain. Corrected Claude-plan baseline claims about Kanban, existing workspaces, contact directory, bulk settlement and agreement clause gaps. Roadmap covers Phases 0–7 with dependencies and acceptance gates.
- Verification: Parsed original DOCX text and XLSX XML; decoded shared strings and reviewed workbook schemas/statuses/KPIs without publishing sample-party data. Inspected source-level receipt/payout/status, reconciliation and messaging paths with line references. Direct official dotloop, Follow Up Boss and BoldTrail pages retrieved successfully. Google Search failed 403 SUBSCRIPTION_REQUIRED, so direct fetches were used. `git diff --check -- "PROPERTY_SALES_SAAS_PHASED_PLAN.md" "AGENT_WORK_LOG.md"` returned no output; section inventory confirms all requested areas and eight phases. No browser, live endpoint, DB, build or runtime tests were run for this planning-only task.
- Handoff: Await owner review; recommended first implementation authorisation is Phase 0. Confirm money custody, approved source versions, tax/commission policy, approval staffing and unrelated-company SaaS rollout. Current branch advanced through concurrent commits and dist changes during research; those changes were preserved and are not claimed here. Earlier STARTED entry used the previous session's GPT-5.6 label; this completion identifies the current model.

### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 1 item 14 — bulk settlement onto the /sales engine
- Context: continuing the merged PROPERTY_SALES_SAAS_PHASED_PLAN Phase 1 (plan doc docs/superpowers/plans/2026-09-10-phase1-deals-sales-connect.md). Items 1–12 landed in the prior session (last commit 0092836 retired the /deals money-writing endpoints). Prior session did not log items 1–12 here; this restores the discipline.
- Item 14 (Task 4 remainder): repointed the bulk settlement screen off the weak /deals status-flip path onto the authoritative /sales lock.
  - Backend: `GET /api/deals/settlement/sales-bulk-data` — read-only readiness feed. For each deal with a linked, not-yet-locked /sales settlement it reports status and, for approved ones, runs the same `complianceBlockers` the lock enforces; `ready = approved && no blockers`. Posts no money.
  - Removed the last weak /deals read/flip paths: `GET /:id/settlement` (LIKE `DEAL:<code>` money picture via computeDealMoney) and `POST/GET /settlement/bulk{,-data}` (settled a deal by flipping settlement_status with no /sales posting). Deleted the now-unused `services/dealSettlement.service.js`. Kept DealDisbursement/DealEvent models + deal_events (audit history).
  - Frontend `SalesBulkSettlement.jsx`: loads the readiness feed, lets staff pick only `ready` rows, and locks each via `POST /api/sales/settlements/:id/lock` (the real lock — SoD + blocker checks + journal/status side effects intact); per-row settled/skipped results with the server's reason.
- Verification (live, backend restarted on :50001, new PID 33588): readiness endpoint returns 3 in-flight settlements with honest blocker counts (SSPC-DL-000002 → 9 blockers, SSPC-DL-000005 → 6, the `returned` one not approved) → ready_to_settle:0 correctly. Retired endpoints all 404: `GET /settlement/bulk-data`, `GET /:id/settlement`, `POST /settlement/bulk`. `admin-portal` `npm run build` → ✓ built.
...
### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 1 completion-gate — full lock-to-'sold' harness
- Built `backend/scripts/e2eDealSalesSettlementFull.js` (npm script `test:full`), the completion-gate proof the fast harness stopped short of: drives ONE sale all the way to a locked settlement + 'sold' property through the real /sales API, clearing every compliance blocker. 28 assertions, all green.
- Split: static gates seeded directly as fixtures (sale profile compliance/assessment/agreement flags + trust & agency physical bank accounts; each party's KYC = role profile complete + a verified required national_id doc); every money movement goes through the /sales HTTP API — buyer receipt (full price) → reconcile → submit/review/approve → per-payout pay-out→reconcile→pay (vendor 950k first so the funds check passes, then agency 50k) → lock.
- Proves: settlement LOCKED (200); sales-picture shows locked; property → 'sold'; deal → 'completed'; trust ledger nets to 0; residual 0. Also validates item 14's readiness feed transition against a REAL lockable settlement — honestly ready:false (4 blockers) while payouts pend, ready:true (0 blockers) once all paid. Ledger posting uses the branch default chart accounts (1100/1110/2100/4100/4110), which exist in branch 1.
- Single test user drives review/approve/lock with the documented super_admin separation-of-duties override + written reason. Recipient-bank verification (a separate-duty action one user can't self-approve over HTTP) is flipped to verified as fixture setup.
...
### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 2 — URL-back property-file sections + upload-button fixes
- Settlement Desk follow-ups from owner testing: (1) Record view proof field was a text input → replaced with the minimalist UploadButton ('Upload proof'); (2) Match view — upload worked but Match stayed disabled with no explanation when the bank line wasn't picked; added a hint saying what's missing. Also fixed a pre-existing latent crash on the property file (`completionConversionBlocked` undefined ref) — it lived only in the settlement section removed in the Settlement Desk work, so it's gone in the current build (the report was a stale cached bundle).
...
### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 3 sub-project 4 (layer 3) — Sales SOP deadlines + escalation (SOP trilogy complete)
- Compute-on-read business-day deadlines for sale SOP stages. Committed on air-conditioning/phase-0-duplicate.
- Backend: new utils/businessDays.js (addBusinessDays/businessDaysBetween, weekdays only) + testBusinessDays.js wired into npm test. progressiveSop.service gains SALE_PHASE_SLA (engagement 3/marketing 7/offer 3/settlement 14/closure 7 business days), registry phaseSla (leasing null), slaBusinessDaysFor, applyStageDueDate, stageDeadline (on_track/due_soon/overdue/escalated). due_date stamped when a stage becomes in_progress at seed (workflowProject), unlock (progressiveSop), and advance (project.controller updateStage) — never overwriting an existing date. Both hydrates spread stageDeadline; workQueue adds a 'sop_overdue' group (role prepare, tier + days_overdue).
- Frontend: Workflow section shows 'Due <date>' + colored tier badge on active stages; property-file next-action bar shows an 'N SOP overdue' chip; Work Queue renders the sop_overdue kind (Go → Workflow section).
...
### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 4 sub-project A — RPPS + RPSS sales service-agreement builders
- Mirrored the RPRM/RPTM agreement recipe for the two residential sales service agreements. Committed on air-conditioning/phase-0-duplicate.
- Sources: user-supplied approved V0.2 docx (SSPC-RPPS-01 Purchase / SSPC-RPSS-01 Sale); extracted text committed under docs/superpowers/.
- Backend: seedSalesAgreementCatalogs.js seeds both 13-item Schedule C catalogues into care_services (verticals sale_purchase / sale_sale, prices in base_price, price_type in tags). scripts/genSalesAgreementClauses.js AUTO-GENERATES verbatim 25-clause modules (rppsClauses.js/rpssClauses.js) from the V0.2 text. salesAgreementRender.js = shared getCatalog/computePricing/buildAgreement (TOC + parties + clauses + Schedules A–C dual-price + cost summary + payment schedule + signature slots). rpps/rpss services bind vertical + kind config. salesAgreement.controller (KIND map, purchase/sale) at /api/sales-agreements (mounted server.js + manifest.js): catalog/meta/preview/list/create → SigningEnvelope (related_type sale_purchase_agreement/sale_sale_agreement, kyc_role buyer/seller) + signer (role 'client' — the ENUM lacks buyer/seller; identity carried by kyc_role + related_type + 'Buyer/Seller signature' field label) + signature fields.
- Frontend: one parametric SalesAgreementScreen (kind) + PurchaseAgreements/SaleAgreements wrappers (list + 4-step wizard: Parties → Services & pricing → Engagement → Review & send); nav 'Sale Agreements' (Selling) + 'Purchase Agreements' (Buying); routes.
- Verified: catalogs 13/13 (idempotent); build fns render all 25 clauses + schedules; create → ENV-RPPS/ENV-RPSS envelopes (53KB doc_html) with a client signer + 'Buyer/Seller signature'+'Date' fields + signing_path; wizard reaches a Review rendering the full RPPS doc; npm test 27/0 (+businessDays) + test:full 28/0. RPRM/RPTM untouched; no schema change.
- Deferred (Phase 4 B/C/D): Contracts hub / review queue / reminders / expiry / variations; signing-completion → activation/billing exactly once; Contacts relationships/dedup. Not merged; no PR.

### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 4 sub-project B — Sales Contracts hub
- Committed on air-conditioning/phase-0-duplicate. A read/aggregation + workflow surface over the RPPS/RPSS signing envelopes; reuses the existing signing remind/void endpoints.
- Backend (salesAgreement.controller): GET /api/sales-agreements/contracts groups envelopes into awaiting_signature/expiring_soon/expired/completed/declined_voided (expiry computed read-time from expires_at) with counts + kind/search filters. POST /contracts/:id/variation voids an OPEN agreement (reason 'Superseded by variation') and returns {kind, prefill} from terms; terminal declined/voided → no re-void (start fresh); completed → 409. Routes declared before /:kind.
- Frontend: SalesContracts.jsx (/residential/contracts, Assurance nav) — bucket sections with Remind/Link/Void/Open/Vary per bucket, kind filter + search. Vary → navigate to the RPPS/RPSS wizard with router-state prefill; SalesAgreementScreen opens in build mode seeded from the prefill (+ 'Variation of <code>' note).
- Fix found in browser QA: varying an already-voided agreement 409'd (re-void); createVariation now only voids open states.
- Verified live: buckets split correctly after back-dating expires_at (awaiting/expiring/expired); kind filter; variation voids-open / prefills / 409-completed; hub renders + Vary opens the prefilled wizard with 0 console errors; npm test 27/0 (+businessDays) + test:full 28/0. Signing controller + A's create flow untouched; no schema change.
- Deferred (Phase 4 C/D): signing-completion → activation/billing exactly once; Contacts relationships/dedup. Also deferred: draft/internal-review stage, tracked supersedes_id chain, expiry scheduler. Not merged; no PR.

### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 4 sub-project C — Sales signing → billing wiring
- On RPPS/RPSS signing completion, draft agency-fee invoices from the signed payment schedule, flag the engagement, log an event. Committed on air-conditioning/phase-0-duplicate. Backend-only (no UI change).
- Migration 0110: nullable indexed agreement_envelope_id on the invoices table (PropertyInvoice) — the once-only key. salesAgreementCompletion.onCompleted(envelope,{transaction}) drafts one draft PropertyInvoice + InvoiceItem per terms.payment_schedule stage (verbatim amounts; only the deposit stage gets a due date), idempotent via count(agreement_envelope_id); then best-effort SaleProfile.agreement_status='signed'. Wired into handleEnvelopeCompleted for the two sales related_types inside the completion tx, in a try/catch that never rolls back the signature; logs a property event.
- Bug found + fixed in live QA: generateCode scans MAX(invoice_code), so per-row calls inside one uncommitted tx collided (same code) → unique violation → only 1 invoice + flag skipped. Fixed by fetching the base code once and incrementing the numeric suffix across the batch.
- Verified: idempotency unit test (5/0, wired into npm test); live sign-to-completion on property 62 → 3 draft invoices (deposit due-dated, later not) + SaleProfile signed; re-run drafted 0. npm test (7+5+27) + test:full 28/0 green; WT/tenancy/care completion branches untouched.
- Deferred (Phase 4 D + later): commission/success-fee reconciliation to final price, sending invoices, a dedicated sales-invoice screen, Contacts relationships/dedup. Not merged; no PR.

### 2026-09-11 | Claude Code (Opus 4.8) | COMPLETED | Phase 4 sub-project D — Contacts relationships/duplicates/authorisation (PHASE 4 COMPLETE)
- Committed on air-conditioning/phase-0-duplicate. Expands the existing Contacts screen; additive.
- Backend: migration 0111 adds authorisations JSON to contacts (+CONTACT_FIELDS whitelist so PUT saves it). GET /api/contacts/:id/relationships aggregates the sales footprint (PropertyDeal seller/owner, SaleOfferParty→offer, SaleTransactionParty→txn, PartyRoleProfile, BuyerMandate, NonCircumventionRecord sale-context, EnvelopeSigner→agreements) grouped by property via a property id-map. GET /api/contacts/:id/duplicates returns other active branch contacts sharing primary_phone/email/national_id/passport_no (matched_on per row, self excluded) — detect+warn only.
- Frontend: new ContactDetail 'Relationships' tab — engagements-by-property (badges + property link), buyer mandates + agreements lists, a possible-duplicates warning panel, and an editable authorised-representatives list (saved via PUT /contacts/:id).
- Fix (pre-existing latent crash surfaced in QA): ContactDetail assumed contact.tags was an array, but MySQL returns JSON columns as strings for some rows → '(r.tags||[]).map is not a function' crashed the screen. Now tags + authorisations are coerced to arrays on load.
...
### 2026-09-12 | Claude Code (Opus 4.8) | COMPLETED | Phase 5 sub-project B — Message templates + delivery status + SMS + suppression
- Committed on air-conditioning/phase-0-duplicate. Sales-inbox-scoped; the provider seam (communication.service sendEmail/sendSMS) is only CALLED, never changed (empty diff).
- Migration 0113: message_templates table; additive communications delivery_status/provider_message_id/delivery_error/sent_at; contacts do_not_email/do_not_sms. MessageTemplate model; Communication/Contact fields; CONTACT_FIELDS whitelist.
- Backend: messageTemplate CRUD (/api/message-templates, branch+scope). salesInbox reply/compose now go through dispatchAndRecord — calls sendEmail/sendSMS and maps the structured result → delivery_status (sent/simulated/failed/suppressed/logged) + provider_message_id/error/sent_at persisted on the Communication; client SMS dispatches via sendSMS; a recipient contact flagged do_not_email/do_not_sms records 'suppressed' without dispatch. thread returns delivery fields; renderTemplate helper.
...
### 2026-09-12 | Claude Code (Opus 4.8) | COMPLETED | Phase 6 sub-project A — Sales reports & analytics + per-property expenses
- Committed on air-conditioning/phase-0-duplicate. Additive read-only aggregation + a small expense store; the Company Expense ledger, settlement/signing/billing engines and the existing sales dashboards are untouched.
- Migration 0114: sale_property_expenses (branch_id, property_id, category ENUM marketing/advertising/staging/photography/other, amount, spent_on, description, created_by) + (branch_id,property_id) index. SalePropertyExpense model.
- Backend: salesReports.controller — per-property expense CRUD (GET/POST /api/sales/properties/:id/expenses, DELETE /api/sales/expenses/:id) + GET /api/sales/reports?from&to[&category] aggregating pipeline (6 stages: count/avg-age/avg-in-stage), conversion (created→agreed→completed + rates), settlement forecast (by month), overdue receivables, fees expected-vs-invoiced-vs-collected (per SaleProfile), response SLA (first outbound Communication vs enquiry, businessDaysBetween≤1) + workload, and expenses/margin. Routes added inside sales.routes (sub-routes of the mounted /api/sales — no new server.js/manifest mount). salesServices.propertyServices rolled expenses + margin (paid−expenses) into the property-file commitments totals and returns expense rows.
...
### 2026-09-12 | Claude Code (Opus 4.8) | COMPLETED | Buyer Service — Phase B (planning + inspection)
- Committed on air-conditioning/phase-0-duplicate. Stage 2 (requirement assessment & planning) + stage 4 (inspection coordination) on the buyer deal file.
- Migration 0117 (additive): buyer_mandates += finance_status ENUM, investment_use, risk_notes, search_strategy, approved_to_proceed + approved_at/by; mandate_candidates += viewing_date, inspection_notes, inspection_photos JSON. Models + whitelists updated.
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED | Full E2E sweep of sell + buy service + production verdict
- Ran the entire harness arsenal against the live API — all green:
  - npm test: testBusinessDays, testSalesAgreementCompletion, testLeadAutomation (12/0), testSalesSettlementCalculations, e2eDealSalesSettlement (27/0).
  - npm run test:full — e2eDealSalesSettlementFull lock→sold (28/0): submit→review→approve→payouts→lock, property SOLD, deal completed, trust nets zero.
  - e2eSalesAgreementFlow (28/0): sale + purchase agreements, MULTI-party signing (2 vendors / 2 buyers, all signatures visible), KYC by admin AND public link + verify, assessment, fee invoices, edit-draft + resend.
  - e2eSellJourneyFull (34/0): full sell money journey — vendor KYC → RPSS signed → assessment → compliance/banks → buyer created (no agreement, KYC verified) → offer → settlement → receipt → reconcile → payouts → LOCK → SOLD → OUR commission invoice recorded PAID.
  - e2eBuyerServiceFull (NEW, 13/0): mandate → approve-to-proceed → candidate → shortlist/inspect → convert → buy deal file aggregate → 8-stage residential_purchase SOP → risk review → non-trust settlement coordination → closure (financial-closure gate).
- Added backend/scripts/e2eBuyerServiceFull.js (KEPT).
- Production verdict: sell + buy service CORE flows are production-ready (all E2E green, money path balances to zero, KYC gates enforced, multi-party signatures render). Finish items before go-live: (1) wire the public website to consume GET /public-website/site (CMS content editable but website rendering handed to website agent); (2) decide on a backfill re-render for PRE-EXISTING agreements (snapshot HTML keeps old duplicate-signature/legacy-checkbox layout — new ones are fixed); (3) witness signer currently requires an email (EnvelopeSigner.email notNull); (4) signed copy is HTML (no server-side PDF/tamper-certificate file — final_pdf_url null); (5) enforce separation of duties with distinct reviewer/approver/locker accounts (tests used single super_admin override); (6) housekeeping — two migrations numbered 0116.

### 2026-09-12 | Claude Opus 4.8 | COMPLETED | Sell journey — FULL assessment SOP coverage added + re-tested
- Feedback: the sell-journey harness only cleared the assessment gate (completed checklist) — it never exercised the full assessment → appraisal → proposal SOP. Re-tested with the full flow; asked to find & fix bugs during checks.
- Extended e2eSellJourneyFull.js assessment step to the full SOP: assessment (create → 9-item checklist → submit → approve) → APPRAISAL (upsert market values min/rec/max + reserve/quick-sale + comparable → submit → approve → generate report) → PROPOSAL (create with approved appraisal + vendor → generate PDF → send to vendor → accept).
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED (BUG FOUND + FIXED) | Workflow checklist coverage + late-SOP unlock reconciliation
- Feedback: I claimed the workflow "works" after only confirming the SOP starts — never worked the stages/checklist items. Corrected.
- Added backend/scripts/e2eWorkflowStages.js: loads a property's properties_sale SOP, prints the full stage+checklist inventory, then ticks EVERY checklist item and completes every unlocked stage in a loop, reporting the final project state and any event-locked stages.
- BUG FOUND (real, production-relevant): a SOP started on an already-advanced property stays permanently blocked. progressiveSop.unlockForEvent only unlocks phases when a lifecycle event fires WHILE the project exists; a workflow started late (e.g. property #115, already SOLD) never received sale_assessment_approved / sale_offer_received / sale_offer_accepted / sale_settlement_locked, so 6 of 10 stages were locked forever with no way to progress.
- FIX (salesSop.controller.js): added reconcileSaleSop() — on ensureSop (create, in-tx) and on getSop (self-heal existing projects), it inspects the property's current state (SaleProfile.assessment_status/approved SaleAssessment, any SaleOffer submitted+, a SaleTransaction, a locked SaleSettlement-via-transaction) and replays the corresponding unlockForEvent calls (idempotent).
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED | Website: Make-an-Offer button + Sold badge on property detail (done for website agent)
- Implemented the website-side UI the earlier handoff left to the website agent (they were busy).
- website-mock/src/services/api.js: getPropertyById + getProperties now expose saleStatus (from p.status), isSold, canOffer; added websiteApi.submitPropertyOffer → POST /public-website/offers.
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED | Sidebar duplicate fix ("shows doubles") + duplicate audit
- Duplicate audit across the whole nav (consoles.js + Layout.jsx): the only genuine render-duplicate was the MAIN sidebar's Commercial and Rural groups — each had a `to:` on the parent header AND a first child pointing to the same route (/commercial/sell, /rural/sell), so the Sell dashboard rendered twice (once as the navigating header, once as a child). Residential was already expand-only.
  FIX (Layout.jsx): made Commercial + Rural expand-only (removed the parent `to`), matching Residential — the "Sales / Sell Dashboard" child still links there, no feature lost.
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED | Full PM journey via COOKIE session + both agreements + both portals; BUG: mgmt fee wrong folio sign
- NEW backend/scripts/e2eFullPmCookie.js (28/0): the ENTIRE PM journey driven through the la_admin_token COOKIE session (no bearer) — the real UI auth path. Covers: cookie /auth/me; landlord contact+client+property; landlord RPRM signed (landlord+SeventhSky+witness) → completed → mgmt fee schedule; website tenant application → verify → approve → convert to tenancy (deposit+folios); tenant RPTM signed; rent invoice → pay; BULK collect via COOKIE (the previously-broken path, now paid=1 failed=0); owner statement; provision + log in to LANDLORD portal (portfolio/statements/approvals load) and TENANT portal (my tenancy/invoices load); quick renewal; end tenancy.
...
### 2026-09-12 | Claude Opus 4.8 | COMPLETED | SSLCommerz online-collection PATHS built (ready for keys); manual stays
- Request: build the SSLCommerz paths now so they're ready to receive the store id/pass + public URL later; keep BOTH manual + online everywhere.
- Reused the existing sales SSLCommerz service (initiate/validate/refund/query + config) — exported config + validateCollection.
- NEW services/sslCommerzInvoice.service.js: initiateInvoiceCollection({invoice, contact}) — generic SSLCommerz session for ANY PropertyInvoice's balance (rent, tenant invoices, our agency fees), invoice-specific callback URLs, value_a=invoice.id.
- NEW controllers/invoiceGateway.controller.js: payLink (auth) creates the session → { gateway_url, tran_id }; public success/ipn RE-VALIDATE server-to-server then record the payment by reusing invoicing.recordPayment IN-PROCESS (so folio + owner fees + reconciliation stay correct, no duplicate accounting); idempotent by tran_id; fail/cancel redirect back. Manual /:id/payments untouched.
- Routes: POST /api/invoices/:id/pay-link (auth, in invoicing.routes) + public /api/invoice-pay/sslcommerz/{ipn,success,fail,cancel} (new invoiceGateway.routes) mounted in server.js AND manifest.js.
...
### 2026-09-14 | Claude Opus 4.8 | COMPLETED | New Interior service line — Furniture & Styling Consultation (own console + routes)
- Request: continue interior sub-lines; check SOP/agreement and add new modules only if needed.
- Source: `.../Interior Design/Furniture & Styling Consultation/` — read SOP (v1.0) + CSA (SSPC-FSCS-CSA-01 v0.2). SOP is the same 4-phase interior workflow (Consultation & Styling Assessment → Furniture & Styling Planning → Implementation → Completion & Handover) with suppliers + variations → NO new module needed; shared engine reused as-is.
- Backend: `furniture_styling_consultation` line (FSCS-* codes) in serviceLines.js; reuse RIDS stages/closure; `FSCS_PACK` (24 clauses + Schedules A–D) in wtCustomerAgreement.service; FSCS work-order doc pack; `seedFurnitureStylingCatalogue.js` (30-item schedule, seeded). Reports/contacts already covered by isInteriorLine().
- Frontend: `furnitureStylingConsole` (fuchsia accent, no Providers/AMC, Variations + Suppliers) in CONSOLES; App.jsx `/furniture-styling-consultation/*` routes (48) wrapped in `FurnitureStylingConsole`; InteriorServiceConsole export; common.jsx UI profile (styling assessment vocabulary); api.js header map; SalesContacts scope; Layout launcher entry.
- Verification: `node scripts/e2eFurnitureStyling.js` 20 PASS / 0 FAIL (client → CSA[FSCS] → signed → project auto-opens + Schedule C invoices FSCI- → variation FSCW-V- → approve → completion sign-off/variations on → supplier/bill/part-pay/payables → provider & AMC refused → isolation). admin build clean.
- Handoff: Furniture & Styling Consultation live at /admin/furniture-styling-consultation (fuchsia). Interior family now 5 live consoles (Residential, Fitness Room, Commercial, Custom Fit-Out, Furniture & Styling). Remaining: Prayer Room, Space Planning & Renovation.

### 2026-09-14 | Claude Opus 4.8 | COMPLETED | New Interior service line — Muslim Prayer Room Interior Design (own console + routes)
- Request: continue interior sub-lines; add new modules only if the SOP/agreement needs them.
- Source: `.../Interior Design/Prayer Room/` — read SOP (SSPC-MPRIDS-SOP-01 v0.1) + CSA (SSPC-MPRIDS-CSA-01 v0.2). Same 4-phase interior workflow with suppliers + variations → NO new module needed; shared engine reused. Prayer-specific vocabulary (Qibla orientation, Wudu facilities, prayer carpet, musallah layout, male/female areas) captured via config UI + assessment checks + CSA pack.
- Backend: `prayer_room_interior_design` line (MPRIDS-* codes) in serviceLines.js; reuse RIDS stages/closure; `MPRIDS_PACK` (24 clauses + Schedules A–D) in wtCustomerAgreement.service; MPRIDS work-order doc pack; `seedPrayerRoomCatalogue.js` (34-item schedule, seeded). Reports/contacts already covered by isInteriorLine().
- Frontend: `prayerRoomInteriorConsole` (deep-emerald accent, no Providers/AMC, Variations + Suppliers) in CONSOLES; App.jsx `/prayer-room-interior-design/*` routes (48) wrapped in `PrayerRoomInteriorConsole`; InteriorServiceConsole export; common.jsx UI profile (Qibla/Wudu/musallah assessment vocabulary); api.js header map; SalesContacts scope; Layout launcher entry.
- Verification: `node scripts/e2ePrayerRoom.js` 20 PASS / 0 FAIL (client → CSA[MPRIDS] → signed → project auto-opens + Schedule C invoices MPRIDI- → variation MPRIDW-V- → approve → completion sign-off/variations on → supplier/bill/part-pay/payables → provider & AMC refused → isolation). admin build clean.
- Handoff: Prayer Room Interior Design live at /admin/prayer-room-interior-design (deep emerald). Interior family now 6 live consoles. Remaining: Space Planning & Renovation.

### 2026-09-14 | Claude Opus 4.8 | COMPLETED | New Interior service line — Space Planning & Renovation (own console + routes) — FINAL of 7
- Request: finish the interior set with Space Planning & Renovation.
- Source: `.../Interior Design/Space Planning and Renovation/` — read SOP (SSPC-SPRS-SOP-01 v0.1) + CSA (SSPC-SPRS-CSA-01 v0.2). Same 4-phase interior workflow with suppliers + variations → NO new module; shared engine reused. Vocabulary: space planning, layout optimisation, renovation fit-out.
- Backend: `space_planning_renovation` line (SPRS-* codes) in serviceLines.js; reuse RIDS stages/closure; `SPRS_PACK` (24 clauses + Schedules A–D) in wtCustomerAgreement.service; SPRS work-order doc pack; `seedSpacePlanningCatalogue.js` (36-item schedule, seeded). Reports/contacts already covered by isInteriorLine().
- Frontend: `spacePlanningConsole` (amber-orange accent, no Providers/AMC, Variations + Suppliers) in CONSOLES; App.jsx `/space-planning-renovation/*` routes (48) wrapped in `SpacePlanningConsole`; InteriorServiceConsole export; common.jsx UI profile; api.js header map; SalesContacts scope; Layout launcher entry.
- Verification: `node scripts/e2eSpacePlanning.js` 20 PASS / 0 FAIL (client → CSA[SPRS] → signed → project auto-opens + Schedule C invoices SPRI- → variation SPRW-V- → approve → completion sign-off/variations on → supplier/bill/part-pay/payables → provider & AMC refused → isolation). admin build clean.
- Handoff: INTERIOR DESIGN SOLUTIONS FAMILY COMPLETE — 7 live consoles: Residential, Fitness Room, Commercial, Custom Design & Fit-Out, Furniture & Styling, Prayer Room, Space Planning & Renovation. All share the one ops engine (project stages, assessment, variations, costing/suppliers, invoicing, auto-open project on CSA signing), each with its own console/route/theme/CSA/catalogue. The isInteriorLine() helper means reports + contacts scope every interior line with no per-line edits.

### 2026-09-17 | Claude Opus 4.8 | COMPLETED | Commercial Sales service line (own console + routes, commercial agreements, isolated)
- Request: read Commercial/Sale folder (SOPs, agreements, excels); build Commercial Sale like Residential Sell (shared engine so residential-sell changes auto-apply), but showing NO residential content; connect website commercial-sales endpoints; main dashboard "Commercial ▸ {Sale, Rent}" (Rent = placeholder this pass).
- Source: `C:\Users\ADMIN\Downloads\Commercial-...\Commercial\Sale\` — Commercial Property Sale Service Agreement V0.2 (CPSS) + Purchase Service Agreement V0.2 (CPPS) + SOP + checklist xlsx. SOP is the same buyer→due-diligence→settlement→commission pipeline the sales engine already runs; xlsx is a checklist/CRM-workflow template → no new module.
- Key finding: the sales sell/buy/enquiry engine is ALREADY category-parametrized (PropertySellDashboard/DealsBoard/SalesEnquiries/SalesProperties take `category`); Property.category is an ENUM. So "residential changes auto-apply to commercial" holds. The gap was the AGREEMENT layer (residential-only RPPS/RPSS) + no category isolation on agreements + a thin commercial route scaffold.
- Backend:
  - `genSalesAgreementClauses.js`: added Commercial jobs; also fixed region end to cut at SIGNATURES (kept residential output byte-identical). Generated `cpssClauses.js` (CPSS, 25) + `cppsClauses.js` (CPPS, 25) verbatim from the V0.2 docs.
  - `salesAgreementSchedules.js`: added `sale_commercial` (CPSS) + `purchase_commercial` (CPPS) Schedule A/B/D taxonomies.
  - `salesAgreementRender.js`: added `header_label` cfg (commercial shows "Seventh Sky Commercial Property Services").
  - New `cpssAgreement.service.js` / `cppsAgreement.service.js` (verticals sale_sale_commercial / sale_purchase_commercial, SSPC-CPSS-01 / SSPC-CPPS-01).
  - `salesAgreement.controller.js`: category-aware REGISTRY[category][kind]; distinct related_types `commercial_sale_agreement` / `commercial_purchase_agreement`; contracts/list/meta/variation all category-scoped → residential & commercial agreements never mix.
  - Completion/fee hooks extended to the commercial related_types: partyRoleActivation, salesAgreementCompletion, wtAgreementCompletion (PDF/email), agencyFees, invoicing SCOPE_RELATED, buyerMandate, sales.controller property-file — all property-scoped so no residential leakage.
  - `seedCommercialSalesCatalogs.js`: Schedule C catalogues (CPSS 8, CPPS 9) seeded.
  - Website: publicWebsite listings already filter ?category=commercial&listing_type=sale and salesEnquiry routes by property.category → commercial enquiries already reach the commercial desk. Verified, not rebuilt.
- Frontend:
  - `paths.js`: registered `commercial: '/commercial'` so every sell-dashboard click stays inside /commercial/* (isolation).
  - `consoles.js`: COMMERCIAL_NAV + COMMERCIAL_BUYER_NAV + commercialConsole/commercialBuyerConsole (sky accent); registered in CONSOLES.
  - `CommercialConsole.jsx` (+ CommercialBuyerConsole export).
  - `App.jsx`: full /commercial/* route set mirroring residential (sell console + buyer console), category="commercial"; removed the 3 old flat commercial routes; `/commercial/rent` placeholder.
  - `SalesAgreementScreen.jsx`: category-aware (passes ?category to all API calls + property combos); PurchaseAgreements/SaleAgreements forward category.
  - `Layout.jsx`: Commercial group → { Sale (/commercial/sell), Rent (/commercial/rent placeholder) }.
- Verification: `node scripts/e2eCommercialSales.js` 13 PASS / 0 FAIL (commercial meta CPSS/CPPS + commercial org, CPSS-* catalog, preview SSPC-CPSS-01 with commercial header & no residential branding, draft ENV-CPSS-, and ISOLATION both ways in list + contracts hub). Residential unchanged (RPSS meta; render still "Residential Property Services", single Signatures; rpps/rpssClauses byte-identical). admin build clean.
- Handoff: Commercial Sale live at /admin/commercial (Commercial ▸ Sale), sky theme, own console, commercial CPSS/CPPS agreements, fully isolated from residential, wired to the website commercial-sale listings/enquiries. Commercial RENT (Rental Mgmt + Tenancy Mgmt from the Rent folder) is the next pass — menu placeholder in place.

### 2026-09-17 | Claude Opus 4.8 | COMPLETED | Editable price schedules for property Sales/Rental lines (like the WT catalogue)
- Request: /admin/air-conditioning/catalogue has a price-schedule editor but there's no equivalent for residential sales, rentals, buys and commercial sales — first check what the services one does, then build accordingly.
- Checked: the WT/AC "Price Schedule" (screens/watertank/Catalogue.jsx → /api/wt-catalogue) edits ServiceItem rows scoped by the service-line's catalogue_vertical (standard price + usage + history). The sales/rental Schedule C catalogues (verticals sale_sale/RPSS, sale_purchase/RPPS, residential_pm/RPRM, tenancy_mgmt/RPTM, sale_sale_commercial/CPSS, sale_purchase_commercial/CPPS) were seed-only — no edit UI/endpoint.
- Backend: new `salesCatalog.controller.js` + `/api/sales-catalog` routes — whitelisted to the six property verticals. GET /schedules?scope= (list + counts), GET /?vertical= (items + summary), PATCH /:id (edit standard price / name / unit / price type / label), POST / (add item). Edits ServiceItem directly, so a new standard price flows straight into what quotations & agreements price from (verified: PATCH CPSS-001 → agreement catalog reflected it). Mounted in server.js + manifest.js.
- Frontend: new `SalesPriceSchedule.jsx` (scope prop) — schedule selector (sky-pill tabs), summary KPIs, inline-editable table (price type fixed/from/percent/included + label), add-item. Routes /residential/price-schedule (scope=residential → Sale/Buy/Rental Mgmt/Tenancy) and /commercial/price-schedule (scope=commercial → Sale/Buy); "Price Schedule" nav item added to the Residential & Commercial console Selling groups.
- Verified: endpoint smoke test (schedules 6 verticals w/ counts, list, PATCH persists into the agreement catalog); admin build clean.
- Handoff: Residential ▸ Price Schedule and Commercial ▸ Price Schedule now let staff view/update standard prices for every property sale/rental/buy schedule, exactly like the service-line catalogues.

### 2026-09-17 | Claude Opus 4.8 | COMPLETED | Sales agreements redesigned to match the Property Management design (window + PDF)
- Request: the residential & commercial sales agreements (sale + purchase) render in the old plain style, not the PM/Tenancy "luxury" design — make all agreement windows and PDFs look like the PM agreements. (Also committed the parallel Antigravity agreement-hub work first: 4a3f8af.)
- Checked: rprmAgreement.service (PM) renders a dedicated cover page, a 1-page Table of Contents, Plus Jakarta Sans, navy schedule badges and a card-style execution block. salesAgreementRender (RPPS/RPSS/CPPS/CPSS) used a plainer Georgia/inline-TOC layout. The sales agreement preview window, signed document and PDF all derive from this one HTML.
